...
function projectPatternOnPointPath(pts, pattern) {
    ...
    const repeatIntervalPixels = pattern.repeat;
    const startOffsetPixels = pattern.offset;
    const endOffsetPixels = pattern.endOffset;

    // 2. generate the positions of the pattern as offsets from the path start
    const positionOffsets = [];
//start xml调整
    if(totalPathLength<startOffsetPixels)
...
//end xml调整
    ...
    return positionOffsets.map(positionOffset => {
        ...
        return {
            pt: segment.a,//interpolateBetweenPoints(segment.a, segment.b, segmentRatio),
