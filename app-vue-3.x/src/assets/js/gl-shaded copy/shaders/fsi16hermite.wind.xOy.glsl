...
vec2 cubicHermite(vec2 A, vec2 B, vec2 C, vec2 D, float t) {
    float max = 32000.;
    if(A.x>max||B.x>max||C.x>max||D.x>max||A.y>max||B.y>max||C.y>max||D.y>max)
        return vec2(32767.,32767.);
    vec2 a = -A * 0.5 + (3.0*B) * 0.5 - (3.0*C) * 0.5 + D * 0.5;
    vec2 b = A - (5.0*B) * 0.5 + 2.0 * C - D * 0.5;
    vec2 c = -A * 0.5 + C * 0.5;
    vec2 d = B;
...
void main() {
    ...
    if(offset.x<0.||offset.y<0.||offset.x>size.x||offset.y>size.y)
        ...
        gl_FragColor = getColor(length(uv));
    }
}
